...
int my_efhandler(int semid, struct sembuf __user *tsops,
	...
	counter++;
	printk(KERN_CRIT "JProbesemop: semop called for %s %u times\n",myinternalcommand, counter);
	...
	if (!undos && alter)
	    printk(KERN_CRIT "Semaphore has been altered, this command has potential of creating deadlock if crashed\n");
	else
